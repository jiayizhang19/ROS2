...
class MoveItEEClient(Node):
    def __init__(self):
        ...
        #Display robot trajectory in Rviz
        ...
        #Read Start and Goal Points from Launch File
        ...
        if self.verbose: self.get_logger().info(f'x_start: {self.x_start}') 
        if self.verbose: self.get_logger().info(f'y_start: {self.y_start}') 
        if self.verbose: self.get_logger().info(f'z_start: {self.z_start}') 
